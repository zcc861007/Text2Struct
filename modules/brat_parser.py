...
def fragment_entity(e: Entity) -> Entity:
    e = deepcopy(e)
    ...
    for m in matches:
        frag_start = start
        while e.text[frag_start].isspace():
            frag_start += 1
        frag_end = m.start()
        while e.text[frag_end - 1].isspace():
            frag_end -= 1
        if frag_start < e.end and frag_start < frag_end:
            spans.append((frag_start + offset, frag_end + offset))
            fragments.append(e.text[frag_start:frag_end])
            start = m.end()

    frag_start = start
    if frag_start < len(e.text):
        while e.text[frag_start].isspace():
            frag_start += 1
        frag_end = len(e.text)
        while e.text[frag_end - 1].isspace():
    ...
    return Entity(
        id=e.id,
        type=e.type,
        span=tuple([(i[0], i[1]) for i in spans]),
        text=" ".join(fragments),
    )


def get_fragmented_line(e: Entity) -> str:
    r = []
    for s in e.span:
        r.append(f"{s[0]} {s[1]}")
    rs = ";".join(r)

    return f"{e.id}\t{e.type} {rs}\t{e.text}"


def dumps(doc: Document, fragment_newlines: bool = True) -> str:
    lines: List[str] = []
    existing_eids = set()
    for e in doc.entities:
        existing_eids.add(e.id)
        line = ""
        while e.text[0].isspace():
            s = list(map(lambda x: list(x), e.span))
            s[0][0] += 1
            e.span = tuple(map(lambda x: (x[0], x[1]), s))
            e.text = e.text[1:]
        if "\n" in e.text and fragment_newlines:
            fragmented_entity = fragment_entity(e)
            line = get_fragmented_line(fragmented_entity)
        elif e.is_fragmented() and fragment_newlines:
            line = get_fragmented_line(e)
        else:
            line = f"{e.id}\t{e.type} {e.start} {e.end}\t{e.text}"
        lines.append(line)
    for r in doc.relations:
        # remove relations with missing arg1 or arg2
        if r.subj in existing_eids and r.obj in existing_eids:
            line = f"{r.id}\t{r.type} Arg1:{r.subj} Arg2:{r.obj}"
            lines.append(line)
    for a in doc.attributes:
        if a.target in existing_eids:
            line = f"{a.id}\t{a.type} {a.target}"
            if a.values:
                line += " " + " ".join(a.values)
            lines.append(line)
    return "\n".join(lines)
...
def dump(doc: Document, path: str, fragment_newlines: bool = True):
    with open(path, mode="w", encoding="utf-8") as fp:
        fp.write(dumps(doc, fragment_newlines=fragment_newlines) + "\n")
